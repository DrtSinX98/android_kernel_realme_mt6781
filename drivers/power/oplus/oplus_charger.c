...
extern void oplus_link_temp_check(struct oplus_chg_chip *chip);
extern int is_spaceb_hc_project(void);
extern int oplus_batt_id;
static bool pdqc_to_normal = false;
static int qc_count = 0;
#define SOC_DOWN_1_COUNT_LIMIT	1
...
extern void oplus_voocphy_pm_qos_update(int new_value);
...
#endif
/*#if defined(VENDOR_EDIT) && defined(CONFIG_OPLUS_CHARGER_MTK6781)//zhaojianchuang
	POWER_SUPPLY_PROP_LINK_TEMP,
#endif*/
#ifdef CONFIG_OPLUS_CALL_MODE_SUPPORT
		...
		enum power_supply_property psp)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_SHIP_MODE:
			...
			break;
#endif
/*#if defined(VENDOR_EDIT) && defined(CONFIG_OPLUS_CHARGER_MTK6781)
	 //zhaojianchuang@ODM_HQ.BSP.CHG, 2020/04/17, add link ntc temp
		case POWER_SUPPLY_PROP_LINK_TEMP:
			if (chip)
				val->intval = chip->link_temp;
			else
				val->intval = 25;
			break;
#endif*/
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CALL_MODE:
			...
			if (is_spaceb_hc_project() == 1) {
				chg_err("chip->calling_on = %d\n", chip->calling_on);
				oplus_chg_set_camera_on(chip->calling_on);
			}
			break;
#endif
#ifdef CONFIG_OPLUS_SHORT_C_BATT_CHECK
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		...
		if(!chip->charger_exist && (chip->charger_volt < 4500)) {
			if (chip->ui_soc == 0) {
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	if (is_spaceb_hc_project() == 1) {
		char node_name[128];
		sprintf(node_name, "qcom,iterm_ma%d", oplus_batt_id);
		rc = of_property_read_u32(node, node_name, &chip->limits.iterm_ma);
		if (rc < 0) {
			chip->limits.iterm_ma = -EINVAL;
		}
	}
	...
	if (is_spaceb_hc_project() == 1)
		chip->smart_charging_screenoff = true;
	...
	chip->limits.temp_normal_vfloat_mv = 4100;
#else
	...
	if (is_spaceb_hc_project() == 1) {
		char node_name[128];
		sprintf(node_name, "qcom,temp_warm_fastchg_current_ma%d", oplus_batt_id);
		rc = of_property_read_u32(node, node_name,
				&chip->limits.temp_warm_fastchg_current_ma);
		if (rc < 0) {
			chip->limits.temp_warm_fastchg_current_ma = -EINVAL;
		}
...
#ifdef CONFIG_OPLUS_CHARGER_NONTC
	chip->limits.normal_vfloat_sw_limit = 4100;
#else
...
#ifdef CONFIG_OPLUS_CHARGER_NONTC
	chip->limits.normal_vfloat_over_sw_limit = 4100;
#else
	...
	if(is_spaceb_hc_project() == 2){//18W
		chip->limits.charger_hv_thr = 10000;
	}
	...
	if(is_spaceb_hc_project() == 2){//18W
		chip->limits.charger_recv_thr = 9700;
	}
	...
	if (is_spaceb_hc_project() == 1) {
		char node_name[128];
		sprintf(node_name, "qcom,ffc_normal_vfloat_sw_limit%d", oplus_batt_id);
		rc = of_property_read_u32(node, node_name,
				&chip->limits.ffc1_normal_vfloat_sw_limit);
		if (rc < 0) {
			chip->limits.ffc1_normal_vfloat_sw_limit = 4450;
...
#ifdef CONFIG_OPLUS_CHARGER_NONTC
chip->limits.ffc_temp_normal_vfloat_mv = 4100;
#else
	...
	if (is_spaceb_hc_project() == 1) {
		char node_name[128];
		sprintf(node_name, "qcom,default_iterm_ma%d", oplus_batt_id);
		rc = of_property_read_u32(node, node_name,
				&chip->limits.default_iterm_ma);
		if (rc < 0) {
			chip->limits.default_iterm_ma = 100;
...
#ifdef CONFIG_OPLUS_CHARGER_NONTC
	chip->limits.default_normal_vfloat_sw_limit = 4100;
	chip->limits.default_temp_normal_vfloat_mv = 4100;
	chip->limits.default_normal_vfloat_over_sw_limit = 4100;
#else
	...
	charger_xlog_printk(CHG_LOG_CRTI,"voocphy_support = %d\n", chip->voocphy_support);
...
#define USB_CDP_CURRENT_MA 1550
#define USB_DCP_CURRENT_MA 2100
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB && charging_current > USB_CURRENT_MA) {
		charging_current = USB_CURRENT_MA;
	} else if (chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP && charging_current > USB_CDP_CURRENT_MA) {
		charging_current = USB_CDP_CURRENT_MA;
	} else if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP && charging_current > USB_DCP_CURRENT_MA
		&& chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_DEFAULT) {
		charging_current = USB_DCP_CURRENT_MA;
	}
...
static void battery_temp_anti_shake_handle(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP) {										/*>53C*/
		...
		chip->limits.hot_bat_decidegc = chip->anti_shake_bound.hot_bound;			/*53C*/
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6781
			if (get_eng_version() == HIGH_TEMP_AGING) {
				chg_err(" CONFIG_HIGH_TEMP_VERSION enable here,charging still\n");
				ret = true;
			}
#endif
	...
	charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY] temp map[%d %d %d %d %d %d %d %d] temp:%d, tbatt_status:%d\n",
			chip->limits.removed_bat_decidegc, chip->limits.cold_bat_decidegc, chip->limits.little_cold_bat_decidegc,
			chip->limits.cool_bat_decidegc, chip->limits.little_cool_bat_decidegc, chip->limits.normal_bat_decidegc,
			chip->limits.warm_bat_decidegc, chip->limits.hot_bat_decidegc, batt_temp, chip->tbatt_status);
	return ret;
}
...
static void oplus_chg_check_tbatt_normal_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_normal_status != tbatt_normal_status) {
		...
		chg_err("tbatt_normal_status status change, [%d %d %d %d %d %d %d]\n", chip->tbatt_normal_status,
...
static bool oplus_chg_check_time_is_good(struct oplus_chg_chip *chip)
{
	...
	pr_err("%s: charging time= %d, max time= %d\n", __func__, chip->total_time, chip->limits.max_chg_time_sec);
...
/*hongzhenglong@ODM.HQ.BSP.CHG 2020/05/12 add for 18W charging current when backlight is on*/
				if (chip->dual_charger_support) {
					chg_err("Disable slave charger!!\n");
...
static void oplus_chg_check_cool_down_ichging(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_allow_reading() == true) {
		...
		chg_err("%s to set input current !!\n", __func__);
		oplus_chg_set_input_current_limit(chip);
	}
}
...
extern bool fast_chg_full;
extern vooc_manager *oplus_vooc_mg;

#define PLUGOUT_CHECK_COUNT 4
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	static int oplus_vooc_fastchg_need_reset = 0;
	static int plugout_check_count = 0;
	if (in) {
		chip->charger_exist = true;
		chip->chging_on = true;
		chip->slave_charger_enable = false;
		oplus_vooc_fastchg_need_reset = 1;
		plugout_check_count = 0;
	} else {
		...
		vbatt_higherthan_4180mv = false;
		chip->pd_svooc = false;
		if (oplus_chg_get_voocphy_support() == true && is_spaceb_hc_project() == 1) {
		...
		if (fast_chg_full == true) {
			chip->voocphy.fastchg_to_normal = true;
		}

		if(is_spaceb_hc_project() == 1){
			if (oplus_vooc_fastchg_need_reset && plugout_check_count++ == PLUGOUT_CHECK_COUNT) {
				oplus_vooc_reset_fastchg_after_usbout();
				oplus_vooc_fastchg_need_reset = 0;
				plugout_check_count = 0;
			}
			if ((oplus_vooc_mg != NULL) && (oplus_vooc_mg->user_set_exit_fastchg == true)) {
...
#endif
	charger_xlog_printk(CHG_LOG_CRTI, "charge loop start\n");
...
extern int sc8547_get_cp_vbat(void);
extern int sc8547_get_adc_enable(u8 *data);
extern int sc8547_set_adc_enable(bool enable);
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_get_voocphy_support() == true && is_spaceb_hc_project() == 1) {
		if(oplus_vooc_get_fastchg_started() == true) {
			...
			if(chip->charger_exist) {
				...
				sc8547_get_adc_enable(&cp_adc_reg);
				if(cp_adc_reg == 0)
					sc8547_set_adc_enable(true);
	...
	if (aicl_delay_count > AICL_DELAY_15MIN) {
		...
		if (oplus_vooc_get_allow_reading() == true) {
			if (is_spaceb_hc_project() == 1) {/*33w*/
				if (chip->dual_charger_support) {
#ifndef CONFIG_OPLUS_CHARGER_MTK6781
/*hongzhenglong@ODM.HQ.BSP.CHG 2021/06/01 modify for slave charging current*/
					chg_err("Disable slave charger!!\n");
					chip->slave_charger_enable = false;
#endif
					chg_err("%s!!\n", __func__);
				...
				chg_err("%s not to check aicl input limit!!\n", __func__);
			}
		}
	} else {
		aicl_delay_count++;
...
{
#ifdef CONFIG_OPLUS_CHARGER_MTK6781
	static int soc_shut_down_count = 0;
	static int soc_down_1_count = 0;
	static int vbatt_lower_than_3310mv_cnt = 0;
	static int cnt = 0;
#endif
	...
	if (chip->charger_exist && chip->batt_exist
		...
		if (oplus_short_c_batt_is_prohibit_chg(chip)) {
			...
			if (chip->ui_soc >= 100) {
				...
				if(chip->notify_code & (1 << NOTIFY_BAT_OVER_VOL)) {
					chip->prop_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
				} else {
		...
		if (chip->smooth_soc == chip->ui_soc) {
			...
			if (soc_down_count >= soc_down_limit) {
				soc_down_count = 0;
/*hongzhenglong@ODM.HQ.BSP.CHG 2021/08/24 add for svooc ui_soc decimals*/
	...
	if (chip->ui_soc < 2) {
		...
		if (oplus_chg_soc_reduce_slow_when_1(chip) == true) {
			if(chip->temperature < 530 && chip->temperature >= 160) {
				if(chip->batt_volt_min < 3310) {
					charger_xlog_printk(CHG_LOG_CRTI, "batt is lower than 3310mv,vbatt_lower_than_3310mv_cnt:%d\n", vbatt_lower_than_3310mv_cnt);
					vbatt_lower_than_3310mv_cnt++;
					if(vbatt_lower_than_3310mv_cnt >= 3) {
						chip->ui_soc = 0;
					}
				} else {
					vbatt_lower_than_3310mv_cnt = 0;
				}
			} else {
				chip->ui_soc = 0;
				vbatt_lower_than_3310mv_cnt = 0;
...
#ifndef CONFIG_OPLUS_CHARGER_MTK6781
/*modify for space-d soc 0 jump to 1 err*/
		} else {
			chip->ui_soc = 1;
#endif
		}
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6781
	if(!chip->charger_exist && (chip->charger_volt < 4500)) {
		if ((vbatt_lowerthan_3300mv == true) && (chip->ui_soc <= 1)) {
			soc_down_1_count++;
			chg_err("vbat < 3300, ui_soc = %d, soc_down_1_count:%d, power off\n", chip->ui_soc, soc_down_1_count);
			if(soc_down_1_count > SOC_DOWN_1_COUNT_LIMIT) {
				soc_down_1_count = 0;
				machine_power_off();
			}
		}
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	...
	} else if (is_spaceb_hc_project() == 2) {//18w
		return;
	}
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if(is_spaceb_hc_project() == 1){//33w
		...
		if (chip->charger_volt <= 7500) {
			...
			if (chip->chg_ops->enable_qc_detect){
				...
				/*chg_err("SPACE-B 18w return_enable_qc_detect !\n");*/
			}
		}
		chg_err("SPACE-B 18w return !\n");
		return;
	}
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_vooc_get_fastchg_started() == false
			...
			if (oplus_chg_get_voocphy_support()
				&& is_spaceb_hc_project() == 1 && (high_vol_light == true ||
		...
		if (!oplus_vooc_get_fastchg_started()
			...
			if (reset_mcu_delay == RESET_MCU_DELAY_15S) {
				...
				if (suspend_charger == false) {
					if (oplus_chg_get_voocphy_support() && is_spaceb_hc_project() == 1) {
						oplus_vooc_switch_mode(NORMAL_CHARGER_MODE);
						oplus_chg_suspend_charger();
						msleep(1500);
						oplus_chg_unsuspend_charger();
...
static int oplus_chg_check_sw_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->batt_volt > vbatt_full_vol_sw) {
		if (chip->icharging < 0 && (chip->icharging * -1) <= chip->limits.iterm_ma) {
			...
			charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY] Battery full by sw count =%d!!\n", chip->sw_full_count);
...
static int oplus_chg_check_hw_full(struct oplus_chg_chip *chip)
{
	...
	chg_err("%s: vbatt_full_vol_hw =%d\n", __func__, vbatt_full_vol_hw);
...
static void oplus_chg_other_thing(struct oplus_chg_chip *chip)
{
	...
	if(is_spaceb_hc_project() == 1){//33w
		if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
			charger_xlog_printk(CHG_LOG_CRTI,"VOOC[ %d / %d / %d / %d / %d ]\n",
			voocphy_get_real_fastchg_allow(), chip->voocphy.fastchg_start, chip->voocphy.fastchg_dummy_start,
			chip->voocphy.fastchg_to_normal, chip->voocphy.fastchg_to_warm/*, g_vooc_chip->btb_temp_over*/);
		}
	}
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	static int pd_chging_flag = false;
	int ret = 0;
	...
	if (chip->dual_charger_support) {
		if (chip->charger_volt > 7500) {
			pd_chging = true;
			if(pd_chging_flag == false && (chip->chg_ops->oplus_chg_get_pd_type() == true)) {
				pdqc_to_normal = false;
				pd_chging_flag = true;
				chip->limits.temp_little_cool_fastchg_current_ma_high
					= chip->limits.pd_temp_little_cool_fastchg_current_ma;
...
static void oplus_chg_pdqc_to_normal(struct oplus_chg_chip *chip)
{
	...
	static int pdqc_to_normal_aicr = false;


	/*charger_xlog_printk(CHG_LOG_CRTI, "vbatt_pdqc_to_5v_thr = %d, vbatt_pdqc_to_9v_thr = %d\n",
			 chip->limits.vbatt_pdqc_to_5v_thr, chip->limits.vbatt_pdqc_to_9v_thr);*/
	...
	if (chip->charger_volt > 7500) {
		...
		pdqc_9v = false;
		if (pdqc_to_normal_aicr == true) {
			chg_err("pdqc_to_normal_aicr success!!\n");
			oplus_chg_set_charging_current(chip);
			oplus_chg_set_input_current_limit(chip);
			pdqc_to_normal_aicr = false;
		}
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if ((chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr && pdqc_9v == true)
			...
			if (ret >= 0) {
				pdqc_9v = false;
				pdqc_to_normal_aicr = true;
				pdqc_to_normal = true;
				if (chip->cool_down_force_5v == true) {
					chg_err("pd cool down to 5v limit return\n");
					return;
				}

				chg_err("PD to normal success");
				chip->limits.temp_normal_fastchg_current_ma
					= chip->limits.default_temp_normal_fastchg_current_ma;
				chip->limits.temp_little_cool_fastchg_current_ma
					= chip->limits.default_temp_little_cool_fastchg_current_ma;
				...
				chip->limits.input_current_charger_ma
					= chip->limits.default_input_current_charger_ma;
				if (chip->charger_volt < 7500) {
					chg_err("pd_to_normal_aicr success!!\n");
					pdqc_to_normal_aicr = false;
					oplus_chg_set_charging_current(chip);
					oplus_chg_set_input_current_limit(chip);
				}
		...
		if ((chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr && pdqc_9v == true)
			...
			if (ret == 1) {
				...
				pdqc_to_normal = true;
				if (chip->cool_down_force_5v == true) {
					chg_err("qc cool down to 5v limit return\n");
					return;
				}

				chg_err("QC to normal success");
				chip->limits.temp_normal_fastchg_current_ma
					= chip->limits.default_temp_normal_fastchg_current_ma;
				chip->limits.temp_little_cool_fastchg_current_ma
					= chip->limits.default_temp_little_cool_fastchg_current_ma;
				...
				chip->limits.input_current_charger_ma
					= chip->limits.default_input_current_charger_ma;
				if (chip->charger_volt < 7500) {
					chg_err("qc_to_normal_aicr success!!\n");
					pdqc_to_normal_aicr = false;
					oplus_chg_set_charging_current(chip);
					oplus_chg_set_input_current_limit(chip);
				}
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	static int qc_chging_flag = false;
	int ret = 0;
	...
	if (chip->charger_volt > 7500 && qc_chging_flag == true) {
		qc_chging = true;
		qc_chging_flag = false;
		qc_count = 0;
/*hongzhenglong@ODM.HQ.BSP.CHG 2021/06/04 add for 18W charging current*/
		oplus_chg_set_input_current_limit(chip);
		oplus_chg_set_charging_current(chip);
		...
		if (qc_count >= 5) {
			...
			qc_count = 6;
			/*pdqc_to_normal = true;*/
			chg_err("QC  config fail count =%d !!", qc_count);
			return;
		}
	}
	chg_err("oplus_chg_qc_config:charger_volt:%d,qc_chging:%d,qc_chging_flag:%d\n", chip->charger_volt, qc_chging, qc_chging_flag);
	if (qc_chging == false
...
static void oplus_chg_dual_charger_config(struct oplus_chg_chip *chip)
{
	...
	chg_err(" enable/disable_slave_cnt = %d/%d, icharging = %d, slave_charger_enable = %d, pdqc_to_normal = %d!!\n",
			enable_slave_cnt, disable_slave_cnt, chip->icharging, chip->slave_charger_enable, pdqc_to_normal);
	...
	if (chip->slave_charger_enable == false) {
		...
		if (chip->slave_charger_enable ==  true) {
			if ((chip->icharging < 0 && (chip->icharging * -1) < chip->slave_chg_disable_ma) || (pdqc_to_normal == true)) {
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	/*zhaojianchuang@ODM.HQ.BSP.CHG 2021/05/08 modify link_temp*/
	charger_xlog_printk(CHG_LOG_CRTI," battery_id : %d\n", oplus_batt_id);
	if(is_spaceb_hc_project() == 1){
		oplus_link_temp_check(chip);
	}
...
bool oplus_chg_get_otg_online(void)
{
	if (!g_charger_chip) {
		...
		printk("%s : %d otg_online=%d\n", __func__, __LINE__, g_charger_chip->otg_online);
...
static const int cool_down_current_limit_onebat[CURRENT_LIMIT_COUNT] = {1200, 1500, 2000, 1500, 2000, 2000};
static const int cool_down_current_limit_vooc[CURRENT_LIMIT_COUNT] = {1500, 1500, 2000, 2500, 3000, 3500};
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "[OPLUS_CHG]val->intval = [%d], shell_temp = [%d], set shell_temp_down = [%d].\n",
	...
	subtype = oplus_vooc_get_fast_chg_type();
	charger_xlog_printk(CHG_LOG_CRTI, "[OPLUS_CHG]subtype = [%d]\n", subtype);
	if (!val) {
		if (c_level_index <= 0) {
			chip->cool_down = 0;
	...
	case CHARGER_SUBTYPE_QC:
		...
		chip->cool_down_force_5v = false;
		chip->chg_ctrl_by_cool_down = true;
		if (subtype == CHARGER_SUBTYPE_FASTCHG_VOOC && oplus_chg_get_voocphy_support()) {
			chip->screenoff_curr = val;
		}
		break;
	default:
...
/*guohuiqing@ODM.HQ.BSP.CHG 2021/06/16	change for level-7 use default setting */
	if (!val || (val > CURRENT_LIMIT_COUNT)) {
		chip->cool_down = 0;
		chip->limits.input_current_charger_ma = chip->limits.default_input_current_charger_ma;
		chip->limits.input_current_vooc_ma_high = chip->limits.default_input_current_vooc_ma_high;
		chip->limits.input_current_vooc_ma_warm = chip->limits.default_input_current_vooc_ma_warm;
	...
	charger_xlog_printk(CHG_LOG_CRTI, "val->intval = [%04x], set cool_down = [%d],  cool_down_force_5v = [%d].\n", val, chip->cool_down, chip->cool_down_force_5v);
}
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_chg_override_by_shell_temp(batt_temp)) {
		...
		if(diff < 150 && diff > -150  && batt_temp >= 320 && batt_temp < 530) {
